...
def fetch_twse_prices(date_yyyymmdd, ctx):
    ...
    try:
        ...
        print(f"TWSE price error for {date_yyyymmdd}: {e}")
    return prices

class StockServerHandler(http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
...
if __name__ == '__main__':
    ...
    with socketserver.TCPServer(("", PORT), StockServerHandler) as httpd:
        print(f"Stock P/E Server running on port {PORT} with TTM EPS Multiple Engine...")
        httpd.serve_forever()
