...
target_old = """/* ─── Fetch data (with LocalStorage cache) ───────────────────────── */
...
replacement_new = """/* ─── Fetch data (with LocalStorage cache) ───────────────────────── */
async function fetchStockData(dateStr, forceRefresh = false) {
...
# Restore git clean state first if needed or apply replacement
if target_old in app_js:
    app_js = app_js.replace(target_old, replacement_new)
    print("Cleanly replaced fetchStockData!")
else:
    print("Target string not found, doing git checkout and patch")
